 ...
 * i.e., E is a type-specialized version of {@link org.checkerframework.framework.qual.EnsuresQualifierIf}. The value {@code qualifier}
 * that is necessary for a conditional postcondition specified with {@link org.checkerframework.framework.qual.EnsuresQualifierIf} is
 * specified here with the value {@code qualifier}.
 *
 * <p>The annotation E that is meta-annotated as {@link org.checkerframework.framework.qual.ConditionalPostconditionAnnotation} must
 * have an element called {@code expression} that is an array of {@code String}s of the same format
 * and with the same meaning as the value {@code expression} in {@link org.checkerframework.framework.qual.EnsuresQualifierIf}. E must
 ...
 * annotation. If the annotation E has elements annotated by {@link org.checkerframework.framework.qual.QualifierArgument}, their values
 * are copied to the arguments (elements) of annotation P with the same names. Different element
 * names may be used in E and P, if a {@link org.checkerframework.framework.qual.QualifierArgument} in E gives the name of the
