 ...
 * type-specialized version of {@link org.checkerframework.framework.qual.RequiresQualifier}. The value {@code qualifier} that is
 * necessary for a precondition specified with {@link org.checkerframework.framework.qual.RequiresQualifier} is specified here with the
 * value {@code qualifier}.
 *
 * <p>The annotation R that is meta-annotated as {@link org.checkerframework.framework.qual.PreconditionAnnotation} must have an element
 * called {@code value} that is an array of {@code String}s of the same format and with the same
 * meaning as the value {@code expression} in {@link org.checkerframework.framework.qual.RequiresQualifier}.
 *
 * <p>The established precondition P has type specified by the {@code qualifier} field of this
 * annotation. If the annotation R has elements annotated by {@link org.checkerframework.framework.qual.QualifierArgument}, their values
 * are copied to the arguments (elements) of annotation P with the same names. Different element
 * names may be used in R and P, if a {@link org.checkerframework.framework.qual.QualifierArgument} in R gives the name of the
 ...
 * </code></pre>
 *
 * @see org.checkerframework.framework.qual.RequiresQualifier
 * @see org.checkerframework.framework.qual.QualifierArgument
 */
